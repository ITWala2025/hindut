...
const defaultTheme = {
  container: {
    center: true,
    padding: "2rem",
    // Explicit width-based screens for the container plugin so
    // it doesn't try to use raw media queries (e.g. (display-mode: standalone))
    // which would produce invalid `max-width` values.
    screens: {
      sm: '640px',
      md: '768px',
      lg: '1024px',
  ...
  extend: {
    // pointer and pwa media queries shouldn't be listed as regular
    // screens because Tailwind's container plugin will try to use the
    // screen value as a width and produce invalid `max-width` CSS.
    // We'll register them as custom variants instead (see `plugins` below).
    colors: {
      neutral: {
...
export default {
  ...
  plugins: [
    plugin(function ({ addVariant }) {
      // Allows using `pwa:...`, `pointer-coarse:...`, `pointer-fine:...` variants
